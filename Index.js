const express = require('express');
const cors = require('cors');
const { MongoClient, ServerApiVersion, ObjectId } = require('mongodb');
require('dotenv').config();
const app = express();
const port = process.env.PORT || 5000;

// middlewares
app.use(cors());
app.use(express.json());

// booksData
// Vt9Z63p5oFaJPvwZ


const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.vo9rggr.mongodb.net/?retryWrites=true&w=majority`;

// Create a MongoClient with a MongoClientOptions object to set the Stable API version
const client = new MongoClient(uri, {
    serverApi: {
        version: ServerApiVersion.v1,
        strict: true,
        deprecationErrors: true,
    }
});

async function run() {
    try {
        // Connect the client to the server	(optional starting in v4.7)
        // await client.connect();
        // const bookDataCollection = client.db("emajhonDB").collection("products");
        const bookDataCollection = client.db("bookDatas").collection("data");


        app.get('/booksdata', async (req, res) => {
            const result = await bookDataCollection.find().toArray();
            res.send(result);
        })

        app.get('/singlebookdata/:id', async (req, res) => {
            const id = req.params.id
            query = { _id: new ObjectId(id) }
            const result = await bookDataCollection.findOne(query)
            res.send(result)
        })


        // Send a ping to confirm a successful connection
        await client.db("admin").command({ ping: 1 });
        console.log("Pinged your deployment. You successfully connected to MongoDB!");
    } finally {
        // Ensures that the client will close when you finish/error
        // await client.close();
    }
}
run().catch(console.dir);



app.get('/', (req, res) => {
    res.send('book menegement running');
})

app.listen(port, () => {
    console.log(`Book management running on port ${port}`)
})